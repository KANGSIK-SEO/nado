...
def main():
    ...
    p.add_argument("--out", type=Path, default=Path("infra"))
    a = p.parse_args()
    a.out.mkdir(parents=True, exist_ok=True)
    (a.out / "template.yaml").write_text(T, encoding="utf-8")
    (a.out / "verify.sh").write_text(V, encoding="utf-8")
    (a.out / "verify.sh").chmod(0o750)
    (a.out / "cleanup-checklist.txt").write_text(
