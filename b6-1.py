#!/usr/bin/env python3
## 쉬운 설명: 회원, 책, 분류, 대여를 번호로 연결해 도서관 장부를 만든다.
...
QUERIES = [
    ...
    (
        "04 반납하지 않은 기록 최신순",
        "SELECT * FROM rental WHERE returned_at IS NULL ORDER BY rented_at DESC;",
    ),
    (
        "05 INNER JOIN 책과 분류",
    ...
    (
        "06 INNER JOIN 회원별 대여 기록",
        "SELECT m.name,r.rented_at FROM member m INNER JOIN rental r ON m.id=r.member_id;",
    ),
    (
        "07 LEFT JOIN 대여하지 않은 회원도 포함",
        "SELECT m.name,r.id FROM member m LEFT JOIN rental r ON m.id=r.member_id;",
    ),
    (
        "08 세 테이블을 연결한 대여 상세",
        "SELECT r.id,m.name,b.title,r.status FROM rental r JOIN member m ON m.id=r.member_id JOIN book b ON b.id=r.book_id;",
    ),
    (
        "09 회원별 대여 횟수",
        "SELECT member_id,COUNT(*) AS rental_count FROM rental GROUP BY member_id;",
    ),
    (
        "10 분류별 도서 금액 합계",
        "SELECT category_id,SUM(price) AS total FROM book GROUP BY category_id;",
    ),
    (
        "11 분류별 평균 가격",
        "SELECT category_id,AVG(price) AS average FROM book GROUP BY category_id;",
    ),
    (
        "12 서브쿼리 평균보다 비싼 도서",
    ...
    (
        "13 미반납 오래된 기록을 연체로 변경",
        "UPDATE rental SET status='overdue' WHERE returned_at IS NULL AND rented_at<'2026-09-05';",
    ),
    (
        "14 반납 완료된 연습 기록 한 건 삭제",
        "DELETE FROM rental WHERE id=12 AND status='returned';",
    ),
    (
        "15 대여일 범위 검색을 위한 인덱스: 전체 스캔 대신 범위 탐색",
        "CREATE INDEX idx_rental_date ON rental(rented_at);",
    ),
]
...
def seed_sql():
    ...
    for i in range(1, 13):
        ...
        lines.append(
            f"INSERT INTO member VALUES({i},'회원{i}','member{i}@example.test');"
        )
        lines.append(
            f"INSERT INTO book VALUES({i},'도서{i}',{(i-1)//2+1},{10000+i*1000});"
        )
    for i in range(1, 13):
        returned = "'2026-09-15'" if i % 2 == 0 else "NULL"
        status = "returned" if i % 2 == 0 else "borrowed"
        lines.append(
            f"INSERT INTO rental VALUES({i},{(i-1)//2+1},{i},'2026-09-{i:02d}',{returned},'{status}');"
        )
    return "\n".join(lines) + "\n"
...
def main():
    ...
    if database.exists():
        raise SystemExit("기존 DB 보존: 새 --out 폴더를 지정하세요")
    ...
    with sqlite3.connect(database) as db:
        ...
        for title, query in QUERIES:
            ...
            if cursor.description:
                ...
                result = "인덱스 생성 완료\n" + str(
                    db.execute(
                        "EXPLAIN QUERY PLAN SELECT * FROM rental WHERE rented_at>='2026-09-05'"
                    ).fetchall()
                )
            else:
                result = f"변경 행: {cursor.rowcount}\n" + str(
                    db.execute("SELECT * FROM rental ORDER BY id").fetchall()
                )
            report = title + "\n" + query + "\n" + result + "\n"
            print(report)
            (target / "results" / f"{title[:2]}.txt").write_text(
                report, encoding="utf-8"
            )
        try:
            db.execute(
                "INSERT INTO rental VALUES(999,999,1,'2026-09-01',NULL,'borrowed')"
            )
        except sqlite3.IntegrityError as error:
            (target / "results" / "fk-check.txt").write_text(
                "실제 FK 차단 확인: "
                + str(error)
                + "\n없는 회원 999를 먼저 등록하거나 기존 회원 ID를 사용해야 합니다.",
                encoding="utf-8",
            )
