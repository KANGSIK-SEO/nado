...
def main():
    ...
    if a.safe_mode:status='\n'.join(status.splitlines()[:10]);diff='\n'.join(diff.splitlines()[:200])
    if not status.strip() and not diff.strip():print('[INFO] 변경 사항이 없습니다.');return
    if a.command=='commit': prompt='커밋 제목 72자 이내 1줄과 핵심 변경 불릿 2개를 만들어라.\n'
    else: prompt='첫 줄은 PR 제목, 다음에는 ## Why, ## What, ## How to Test 헤더와 각 1개 이상 불릿을 만들어라.\n'
    print('[INFO] Git status/diff 수집 완료\n[INFO] AI API 요청 중...\n'+ask(prompt+'STATUS\n'+status+'\nDIFF\n'+diff,a.model,a.temperature,a.max_tokens))
if __name__=='__main__':
    try:main()
    except Exception as error:print('[ERROR]',error,file=sys.stderr);sys.exit(1)
