...
class MiniGit:
    ...
    def add_commit(self, message, extra=None):
        ...
        if extra and extra not in parents: parents.append(extra)
        self.counter += 1
        identifier = f'{self.counter:016x}'
        commit = Commit(identifier,message,self.user,datetime.now(timezone.utc).isoformat(),parents)
        self.commits[identifier] = commit
        self.edges[identifier] = list(parents)
        for parent in parents: self.edges[parent].append(identifier)
        ...
        while index < len(queue):
            ...
            for neighbor in merge_sort(self.edges[current]):
                if neighbor not in parents:
                    parents[neighbor] = current; queue.append(neighbor)
        return 'No path'
    ...
    def topological(self):
        ...
        for h,c in self.commits.items():
            for p in c.parents: children[p].append(h)
        queue = [h for h in self.commits if indegree[h]==0]
        index, result = 0, []
        while index<len(queue):
            h=queue[index]; index+=1; result.append(self.commits[h])
    ...
    def display(self, commits):
        return '\n'.join(f'commit {c.hash} ({c.author}, {c.timestamp})\n{c.message}' for c in commits) or 'No commits'

    def execute(self, line):
        try: words = shlex.split(line)
        except ValueError: return 'Invalid args'
        if not words: return ''
        cmd, args = words[0].upper(), words[1:]
        if cmd == 'INIT':
            if len(args)!=1 or not args[0].strip(): return 'Invalid args'
            if self.user is not None: return 'Already initialized'
            self.user, self.branch, self.branches = args[0], 'main', {'main':None}
            return 'Initialized repository.\nCurrent branch: main\nCurrent user: '+self.user
        if self.user is None: return 'Run INIT <user_name> first'
        if cmd in ('BRANCH','SWITCH','COMMIT','ANCESTORS','SEARCH','MERGE'):
            if len(args)!=1 or not args[0].strip(): return 'Invalid args'
        elif cmd=='PATH':
            if len(args)!=2: return 'Invalid args'
        elif cmd=='LOG':
            if len(args)>1 or (args and args[0] not in ('--sort-by=date','--sort-by=author')): return 'Invalid args'
        else: return 'Unknown command: '+words[0]
        if cmd=='BRANCH':
            if args[0] in self.branches: return 'Branch already exists: '+args[0]
            self.branches[args[0]]=self.branches[self.branch]
            return 'Created branch: '+args[0]
        if cmd in ('SWITCH','MERGE'):
            if args[0] not in self.branches: return 'Unknown branch: '+args[0]
            if cmd=='SWITCH': self.branch=args[0]; return 'Switched to branch: '+args[0]
            return self.add_commit('Merge '+args[0],self.branches[args[0]])
        if cmd=='COMMIT': return self.add_commit(args[0])
        if cmd in ('PATH','ANCESTORS'):
            for h in args:
                if h not in self.commits: return 'Unknown commit: '+h
            return self.path(*args) if cmd=='PATH' else self.display([self.commits[h] for h in self.ancestors(args[0])])
        if cmd=='SEARCH':
            if args[0].startswith('--author='):
                ...
                for token in tokens[1:]: candidates.intersection_update(self.keyword.get(token,[]))
                ids=merge_sort(list(candidates))
            return self.display([self.commits[h] for h in ids])
        commits=self.topological()
        if args:
            commits=merge_sort(commits,key=(lambda c:c.timestamp) if args[0].endswith('date') else (lambda c:c.author))
