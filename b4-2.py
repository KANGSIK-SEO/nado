...
def main():
    ...
    p.add_argument("monitor", type=Path)
    p.add_argument("app", type=Path)
    p.add_argument("--out", type=Path, default=Path("reports"))
    a = p.parse_args()
    monitor = a.monitor.read_text(errors="replace")
    app = a.app.read_text(errors="replace")
    a.out.mkdir(parents=True, exist_ok=True)
    kind = (
        "OOM"
        if re.search(r"Memory|OOM|SELF-TERMINATED", app, re.I)
        else ("CPU" if re.search(r"WATCHDOG|CPU", app, re.I) else "Deadlock")
    )
    cpu = re.findall(r"CPU[:= ]+([0-9.]+)", monitor, re.I)
    mem = re.findall(r"MEM(?:ORY)?[:= ]+([0-9.]+)", monitor, re.I)
    report = (
        "[Bug] "
        + kind
        + " 장애 분석\n\n## 1. Description\n- 실제 로그를 근거로 분류한 장애입니다.\n\n## 2. Evidence & Logs\n- monitor 샘플 수: "
        + str(len(monitor.splitlines()))
        + "\n- CPU 최근값: "
        + str(cpu[-5:])
        + "\n- MEM 최근값: "
        + str(mem[-5:])
        + "\n- 앱 마지막 로그: "
        + str(app.splitlines()[-1:] or ["없음"])
    ...
    (a.out / (kind.lower() + ".txt")).write_text(report, encoding="utf-8")
    print(report)
